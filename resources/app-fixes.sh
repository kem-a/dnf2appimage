...
apply_app_fixes() {
    ...
    # Setup fragments resource bind (runtime via bwrap)
    setup_fragments_bwrap "$APPDIR"
}
...
# Fragments needs /usr/share/fragments to be present at runtime
# Create a generic bwrap bind list for AppRun to consume
setup_fragments_bwrap() {
    local APPDIR="$1"

    if [ -d "$APPDIR/usr/share/fragments" ]; then
        local binds_file="$APPDIR/bwrap.binds"
        ...
        if ! grep -qx "dir /usr/share/fragments" "$binds_file" 2>/dev/null; then
            echo "dir /usr/share/fragments" >> "$binds_file"
        fi

        if ! grep -qx "bind usr/share/fragments /usr/share/fragments" "$binds_file" 2>/dev/null; then
            echo "bind usr/share/fragments /usr/share/fragments" >> "$binds_file"
        fi

        echo "Configured bwrap bind for /usr/share/fragments"
    fi
}
